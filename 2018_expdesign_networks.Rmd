...
* Let $Z_i \sim Bern(p)$ for $p \in (0,1)$. Then, a _Bernoulli randomization
...
* Such a strategy restricts to treatment allocation vectors in
  $$\mathcal{Z}^u \equiv \{Z \in \mathcal{Z}:
    \frac{1}{N_1}\sum_{i: Z_i = 1} \lvert \mathcal{N}_i \rvert -
    \frac{1}{N_0}\sum_{i: Z_i = 0} \lvert \mathcal{N}_i \rvert = 0\}$$

* Aim to minimize the contribution of the bias to the conditional MSE.
...
* welp...now, the strategy is married to the chosen model (e.g., normal-sum).
...
* Draw treatment allocation vector $Z$ from original design, accept if
  $\phi(Z) = 1$ and discard if $\phi(Z) = 0$.

## Issues in inference

* Neymanian intervals -- challenging (under restricted randomization) for
  several reasons
  * Asymptotic theory of re-randomization is incompatible with network
     settings:
     ...
     2. constraints must be a function of difference in group means and
        variance-covariance of that vector (proven to not hold).
...
* As a consequence of design unbiasedness and of the increasingly nested
...
* Need a few regularity conditions on $g$ to ensure that
